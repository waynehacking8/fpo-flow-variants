"""
Run all multi-seed experiments sequentially to avoid GPU OOM issues.
This script trains OT, VP, and Cosine flow variants with 3 seeds each.
"""
...
def train_single_run(
    ...
    del agent_state, rollout_state, env
    gc.collect()
...
def main():
    ...
    parser.add_argument("--flow_types", type=str, nargs="+", default=["ot", "vp", "cosine"],
    ...
    for flow_type in args.flow_types:
        print(f"\n{'#'*70}")
        print(f"# Starting {flow_type.upper()} experiments")
        print(f"{'#'*70}")
        ...
        for seed in args.seeds:
            try:
                ...
                flow_results.append(results)
            except Exception as e:
                print(f"Error training {flow_type} seed {seed}: {e}")
                import traceback
                traceback.print_exc()

        # Compute statistics for this flow type
        if flow_results:
            final_rewards = [r["final_reward"] for r in flow_results if r["final_reward"] is not None]
            if final_rewards:
                mean_reward = onp.mean(final_rewards)
                std_reward = onp.std(final_rewards)
                print(f"\n{'='*60}")
                print(f"STATISTICS for {args.env} - {flow_type.upper()}")
                print(f"{'='*60}")
                ...
                stats_path = os.path.join(args.save_dir, f"{args.env}_{flow_type}_stats.json")
                with open(stats_path, "w") as f:
                    json.dump(all_results[flow_type], f, indent=2)
                print(f"Statistics saved to {stats_path}")

    # Final summary
    print(f"\n{'#'*70}")
    print(f"# FINAL SUMMARY for {args.env}")
    print(f"{'#'*70}")

    for flow_type, stats in all_results.items():
        print(f"{flow_type.upper():10s}: {stats['mean']:.2f} ± {stats['std']:.2f}")

    # Save all results
    all_stats_path = os.path.join(args.save_dir, f"{args.env}_all_stats.json")
    with open(all_stats_path, "w") as f:
        json.dump(all_results, f, indent=2)
    print(f"\nAll statistics saved to {all_stats_path}")
